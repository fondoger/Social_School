...
export default class SquareScreen extends React.Component {
  ...
  refreshPosts = () => {
    API.Status.get({ type: 'post' }, (responseJson) => {
      7
      this.setState({
        refreshing: false,
        posts: responseJson,
  ...
  renderPostItem({ index, item, section }) {
    ...
    return (
      <TouchableHighlight onPress={onPress} underlayColor={Theme.activeUnderlayColor}>
        <View style={{
          flexDirection: 'row', borderBottomWidth: 0.5, borderColor: '#ddd',
